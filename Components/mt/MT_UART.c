...
{
  uint8  buf[50];  //保存接收到的数据，最长50字节
  uint8  flag = 0; // 标志是否接收到数据
  uint8  len = 0;  // 记录数据长度
  (void)event;  // Intentionally unreferenced parameter

  while (Hal_UART_RxBufLen(port))  // 串口接收到数据时进入循环
  {
  	flag = 1;   // 标记收到串口数据
    HalUARTRead (port, &buf[len], 1); // 将串口数据记录下来
	len++;      // 已记录的数据字节数
  }
  if (flag == 1){  // 有串口数据时通知App任务
  	pMsg = (mtOSALSerialData_t*)
		osal_msg_allocate(sizeof(mtOSALSerialData_t)+1+len);
	if (pMsg){
		pMsg->hdr.event = CMD_SERIAL_MSG;
		pMsg->msg = (uint8*)(pMsg+1);//把指针移到存放数据的部分
		pMsg->msg[0] = len;          // 第一个位置为长度，后面数据
        osal_memcpy(pMsg->msg +1, buf, len );
		osal_msg_send(App_TaskID,(uint8 *)pMsg); // 给app任务发消息
